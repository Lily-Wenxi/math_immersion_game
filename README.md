...
- Daily SSAT Flash Card check-in (20 words/day) with clean flip cards, synonym/antonym review, word-matched images on flip (`assets/flashcards/<word>.*`) plus generated fallback image when missing, one-click word pronunciation (`🔊 Pronounce`), and an automatic +1 account point when daily 20-word check-in is completed (shared with Math page points).
