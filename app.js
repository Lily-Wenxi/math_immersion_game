...
const challengePanelEl = document.getElementById("challengePanel");
...
function animatePanel(type) {
  if (!challengePanelEl) return;
  const className = type === "good" ? "panel-celebrate" : "panel-encourage";
  challengePanelEl.classList.remove("panel-celebrate", "panel-encourage");
  void challengePanelEl.offsetWidth;
  challengePanelEl.classList.add(className);
}

function showPointsBurst(points) {
  if (!pointsBurstEl) return;
  pointsBurstEl.textContent = `+${points} pts!`;
  pointsBurstEl.classList.remove("show");
  void pointsBurstEl.offsetWidth;
  pointsBurstEl.classList.add("show");
}
...
function submitAnswer() {
  ...
  if (correct) {
    ...
    if (!alreadyCompleted) {
      ...
      showPointsBurst(state.selectedLevel.points);
